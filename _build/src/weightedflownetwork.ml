...
let dfs rg sc sk = 
    let rec _dfs acc delta visited = function
        | []            -> visited
        | (id, (residual_flow, _, direction))::t  -> 
...
let rec update_flow_network fn delta = function
    ...
    | (id1, dir1)::((id2, dir2)::t as tail)   ->    
        ...
        | Opposite  -> update_flow_network (update_edge fn id2 id1 (-delta)) delta tail


(* Busacker-Gowen Algorithm on weighted flow networks

 * wfn : weightedflownetwork
