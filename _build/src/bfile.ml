...
let export bg input_path output_path goal = 
    ...
    let rec loop () =
        try
            ...
            match String.split_on_char ':' line with
                | [name;id] -> Hashtbl.add hashtbl id (name, 0); loop ()
    ...
    (* Get all the right column capacity so that we can display them in the dot file *)
    let _ = e_iter bg 
                (fun id1 id2 (f,c,w) ->
                    if id2 = 9999 then
                        let (name, _) = Hashtbl.find hashtbl (string_of_int id1) in
                        Hashtbl.replace hashtbl (string_of_int id1) (name, c)
                    else ())
    in
    ...
    let f_final id1 id2 (flow, capacity, weight) =
                if flow <> 0 && id1 <> 9998 && id2 <> 9999 then                
                    let (name1, _) = Hashtbl.find hashtbl (string_of_int id1) in
                    let (name2, c) = Hashtbl.find hashtbl (string_of_int id2) in
                    let (color, penwidth) =
                        if      weight = 0 then ("green",   "1.0")
                        else if weight < 3 then ("orange",  "2.0")
                        else                    ("red",     "3.0")
                    in
                    fprintf output_file "\t%s -> \"%s\" [ label = \"%d\" color = \"%s\" penwidth = %s];\n"
                        name1
                        (name2 ^ " (" ^ (string_of_int c) ^ ")")
                        weight
                        color
                        penwidth
                else ()
    in
    let f_intermediate id1 id2 (flow, capacity, weight) =
                (* The only difference is that the flow can be null *)
                if  id1 <> 9998 && id2 <> 9999 then                
                    let (name1, _) = Hashtbl.find hashtbl (string_of_int id1) in
                    let (name2, c) = Hashtbl.find hashtbl (string_of_int id2) in
